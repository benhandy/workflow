...
df.to_csv(target_s3_uri, index=False)  # Save the cleaned dataframe to the target S3 URI
...
with DAG(
   ...
   partition_date = (
    ...
    sql_conn_id="mysql_connection",  # Replace with your actual connection ID
    
    # Set the query value
    query="SELECT * FROM orders;",  # Replace with your actual query as needed
    
    # Use method `get()` requesting the `s3_bucket` name
    s3_bucket=Variable.get("s3_bucket"),  # Replace 's3_bucket_name' with the variable key in Airflow
...
transform_task = PythonOperator(
    ...
    op_kwargs={
        # Use method `get()` requesting the `s3_bucket` name
        "source_bucket": Variable.get("s3_bucket_name"),  # Replace 's3_bucket_name' with the actual variable key
